...
def parse_item_main_exact(code: str) -> Tuple[Optional[float], Optional[float]]:
    ...
    trade_value on the page is shown in 백만원, so divide by 100 to convert to 억원.
    """
    ...
    if table:
        ...
        if len(trs) >= 2:
            ...
            if len(tds) >= 3:
                ...
                val = extract_digits(txt)
                if val is not None:
                    trade_value_eok = val / 100.0

    if trade_value_eok is None:
        # fallback: look for explicit 거래대금 label nearby
        text = soup.get_text('\n', strip=True)
        m = re.search(r'거래대금\s*([\d,]+)', text)
        if m:
            trade_value_eok = float(m.group(1).replace(',', '')) / 100.0
